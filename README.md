...
- **Loading Mask**:黑色半透明遮罩(透明度約 0.3),涵蓋**整個視窗**(含分頁列、登出按鈕),任何分頁在查詢/載入資料表清單/載入欄位清單時都會顯示,期間無法切分頁、登出或做其他操作。遮罩分兩個階段:
  1. **等待資料庫回應**:中下方的 ProgressBar 依實際經過秒數往上跑(搭配文字顯示已等待幾秒),查詢本身的 `SqlCommand.CommandTimeout` 也設為相同秒數(預設 30 秒),逾時未完成會自動中止並跳出錯誤訊息,不會無限期卡住。
  2. **處理查詢結果**:資料庫回應後,遮罩會切換成不歸零的忙碌狀態(文字顯示「資料處理中,請稍候...」),涵蓋接下來在 UI 執行緒上組出 DataGrid 顯示結果的過程——這段排版工作無法搬到背景執行緒,資料量大或欄位多時仍可能讓畫面停頓一下,但遮罩會誠實地蓋住整段時間,不會出現「遮罩已經消失但畫面其實還在忙」的狀況。

### 單一分頁內容(ComparisonTabContent)
- 左側:`compid`/`subcompid`/`select top`(查詢結果筆數上限,預設 100、可輸入 1~1000 的整數,超出範圍或非數字會自動修正)欄位,以及資料表清單(可篩選)。開新分頁、選取資料表時清單/欄位資料都是非同步背景查詢,搭配上述 Loading Mask。
...
- 「查詢」執行後,SQL 會依 `select top` 欄位的數值加上 `SELECT TOP N`,結果以堆疊方式加入下方清單(最多保留 10 筆,超過會捨棄最舊的一筆),每筆結果左側有核取方塊(最多同時勾選 2 筆)。
- 「清空」會先跳出確認對話框,避免誤按。
...
- 查詢結果的 `DataGrid` 為了讓「資料比對」能對任何一列(包含捲動出畫面外的列)標色,關閉了列/欄虛擬化(`EnableRowVirtualization`/`EnableColumnVirtualization = false`),代表每筆結果的所有儲存格都會一次全部建立。資料表欄位數多、或 `select top` 設定的筆數大時,查詢結果送達後的排版過程可能讓畫面停頓數秒(Loading Mask 會涵蓋這段時間,但無法完全避免停頓本身),效能受本機硬體影響較大。
